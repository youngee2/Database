...
LIST varchar2(70) NOT NULL UNIQUE
);
...
select EMPNO,ENAME,SAL FROM EMP WHERE SAL>=1500 AND 2500>=SAL;
select COUNT(*) FROM EMP WHERE ename LIKE 'A%';

select EMPNO,ENAME,JOB FROM EMP WHERE JOB LIKE 'ACCOUNT%' ORDER BY EMPNO

SELECT SYSDATE FROM DUAL;

SELECT empno,ename,sal FROM emp WHERE sal >=1500 AND sal<=2500;

SELECT empno,ename,job FROM EMP  WHERE job='ACCOUNTING' ORDER BY EMPNO;

SELECT empno,ename,job FROM EMP,dept WHERE  emp.deptno=dept.deptno AND dname='ACCOUNTING'  ORDER BY EMPNO ; 

SELECT ENAME,SAL FROM EMP NVL(SAL>2000,SAL=2000);

SELECT ename,sal FROM emp CASE WHEN SAL>2000 THEN 2000 END, sal;

SELECT ename,sal,NVL(sal+comm,0) AS s FROM emp  ORDER BY s desc,sal desc ;
-- 이건 sal+comm 순서 먼저 출력하고 뒤에 sal 위에서 밑으로 출력


SELECT ename,sal nvl(sal<2000,2000) FROM EMP oderby ename ;

SELECT LOC,avg(sal) AS a FROM EMP,dept   GROUP BY loc HAVING SELECT FROM emp WHERE sal ;

SELECT * FROM dept;

SELECT job, count(*),avg(sal),max(sal),min(sal)FROM emp GROUP BY job ORDER BY avg(sal) DESC;

SELECT ENAME ,CASE WHEN sal<2000 THEN 2000  else sal END AS sal FROM emp  ;

SELECT ename,sal ,DECODE(sal,sal<2000,2000) FROM emp ; 

SELECT ename,sal CASE sal WHEN sal>2000 THEN 2000 FROM emp;

SELECT ENAME,SAL CASE SAL WHEN SAL>2000 THEN 2000 FROM EMP;

SELECT ENAME, CASE WHEN SAL<2000 THEN 2000  ELSE SAL END AS SAL FROM EMP;

SELECT LOC, avg(sal) FROM EMP,DEPT  WHERE emp.DEPTNO = dept.DEPTNO  GROUP BY loc ORDER BY avg(sal) desc ;
...
SELECT ENAME,SAL,NVL(SAL+comm,0) AS s FROM EMP ORDER BY s desc;
