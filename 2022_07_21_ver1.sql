...
select word_con from word where word='희희 '
...
CONSTRAINT login_pk PRIMARY KEY (idx) );
...
INSERT INTO login VALUES (idx.NEXTVAL,'green', 'green','green','green');

CREATE TABLE memo (
idx NUMBER (10),
memo_num NUMBER(38,0),
memo_title varchar2(100) NOT NULL,
memo_content varchar2(3000) NOT NULL,
CONSTRAINT memo_pk PRIMARY KEY (memo_num),
CONSTRAINT memo_fk FOREIGN KEY(idx) REFERENCES login(idx) ON DELETE CASCADE
);

CREATE TABLE todolist (
idx number (10),
todo_num NUMBER(38,0),
todo varchar2(80) NOT NULL,
CONSTRAINT todolist_pk PRIMARY KEY (todo_num) ,
CONSTRAINT todolist_fk FOREIGN KEY(idx) REFERENCES login(idx) ON DELETE CASCADE
);
...
word_con varchar2(400) ,
CONSTRAINT word_pk PRIMARY KEY (word_num),
CONSTRAINT word_fk FOREIGN KEY(idx) REFERENCES login(idx) ON DELETE CASCADE 
);
...
INSERT INTO word VALUES ('23',idx.NEXTVAL,'green', 'green');

ALTER TABLE word DROP CONSTRAINT word_fk;

ALTER TABLE login DROP CONSTRAINT login_fk;
...
INSERT INTO word(idx,word_num,word,word_con) values('green',word_num.NEXTVAL,'green',' green') ;

select word_con from word where word='b'

select idx from login where id='green'

select idx from login where id='green'


INSERT INTO word(idx,word_num,word,word_con) values('green',word_num.NEXTVAL,'ads',' dsadas')

select idx from login WHERE id= 'green'

select idx from login where id = 'green'AND pw='green'

 select * from word where idx='green'
 
 INSERT INTO todolist(idx,todo_num,Todo) values('null',todo_num.NEXTVAL,'안뇽')
 
 INSERT INTO todolist(idx,todo_num,Todo) values('green',todo_num.NEXTVAL,'aaaa')
